...
def isSortedR[A](as: Seq[A], ordered: (A,A) => Boolean): Boolean = {
  as match {
    case Seq() => true
    case Seq(_) => true
    case Seq(a, remaining@_*) => ordered(a,remaining.head) && isSortedR(remaining, ordered)
  }
}
...
assert(!isSorted(Array('c','b','c'), (a:Char,b:Char) => a >= b))
...
assert(!isSortedR(Array('c','b','c'), (a:Char,b:Char) => a >= b))
